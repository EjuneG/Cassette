...
> Note: several internal identifiers intentionally keep the old `yesplaymusic` name for data continuity — do **not** rename them. Most important: `background.js` calls `app.setName('yesplaymusic')` at startup, because Electron derives the userData dir (`~/.config/yesplaymusic`) **and** the OS-keyring label that encrypts cookies from `app.getName()`. Renaming package.json `name` to `cassette` silently moved the data dir and logged the user out; the `setName` pin restores it. **`setName` alone is not enough**: import hoisting runs dependency require-time code first, and any dep that calls `app.getPath()` on require (electron-log does) pins userData/sessionData/logs to the package.json name before `setName` executes — v1.3.2 shipped that bug and logged users out again. `background.js` therefore also calls `app.setPath('userData'/'sessionData')` + `app.setAppLogsPath()` explicitly right after `setName`; never remove those lines, and treat any new top-level import that might touch `app.getPath` at require time as suspect. Also kept: Electron `appId` `com.yesplaymusic.app`, IndexedDB name `yesplaymusic`, MPRIS bus name, and the `~/Apps/YesPlayMusic.AppImage` install path (filename kept, dir moved from ~/Applications in 2026-08). Brand-facing names are all "Cassette".
...
# Local install (Linux) — build AppImage and replace ~/Apps/YesPlayMusic.AppImage
